...
def plot_one_box_kpt(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    ...
    #cv2.rectangle(im, c1, c2, (255,255,255), thickness=tl*1//3, lineType=cv2.LINE_AA)
    if label:
        if len(label.split(' ')) > 1:
            ...
            cv2.rectangle(im, c1, c2, [225, 255, 255], -1, cv2.LINE_AA)  # filled
...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    palette = np.ones((15, 3), dtype=np.uint8) * 255
    ...
    for kid in range(num_kpts):
        r, g, b = [255,255,255] #pose_kpt_color[kid]
    ...
    for sk_id, sk in enumerate(skeleton):
        ...
        if pos2[0] % 640 == 0 or pos2[1] % 640 == 0 or pos2[0]<0 or pos2[1]<0:
            continue
        cv2.line(im, pos1, pos2, (255, 255, 255), thickness=2)
